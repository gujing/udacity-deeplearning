...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    target_id_text = [[target_vocab_to_int[word] for word in sent.split() ] 
                      for sent in target_text.split('\n')]
    for sen_id in target_id_text:
        sen_id.append(target_vocab_to_int['<EOS>'])
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    rnn_cell = tf.contrib.rnn.MultiRNNCell([drop_cell] * num_layers)
    output, state = tf.nn.dynamic_rnn(rnn_cell, rnn_inputs, dtype=tf.float32)
    # TODO: Implement Function
    return state
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    outputs, final_state, final_context_state  = tf.contrib.seq2seq.dynamic_rnn_decoder(drop_cell, dynamic_fn_train, inputs=dec_embed_input, sequence_length=sequence_length,
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    outputs, final_state, final_context_state = tf.contrib.seq2seq.dynamic_rnn_decoder(drop_cell, dynamic_fn_inference,
                                           scope=decoding_scope)
    return outputs
